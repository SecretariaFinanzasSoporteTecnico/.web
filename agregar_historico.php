...
// Generamos el folio igual que en Nuevo Servicio
$folio = 'HIST-' . date('Y') . '-' . str_pad(rand(1, 9999), 4, '0', STR_PAD_LEFT);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // CAPTURAMOS EL AÑO QUE EL USUARIO SELECCIONÓ PARA EL HISTÓRICO
    $anio_archivado = intval($_POST['anio_archivado']);
    ...
    if (empty($nombre_solicitante) || empty($descripcion) || empty($falla_reportada)) {
        ...
        // ===== CALCULAR EL SIGUIENTE FOLIO ANUAL PARA ESE AÑO =====
        // (Se bloquea la tabla brevemente para evitar que dos registros
        // simultáneos obtengan el mismo folio)
        $conn->begin_transaction();

        $stmt_max = $conn->prepare("SELECT COALESCE(MAX(folio_anual), 0) AS max_folio FROM servicios WHERE anio_archivado = ? FOR UPDATE");
        $stmt_max->bind_param("i", $anio_archivado);
        $stmt_max->execute();
        $max_folio = $stmt_max->get_result()->fetch_assoc()['max_folio'];
        $stmt_max->close();

        $folio_anual = $max_folio + 1;

        // Insertamos el registro (consulta preparada) con el folio anual calculado
        $stmt_insert = $conn->prepare("INSERT INTO servicios (
            ...
            soporte_recibe, accion_realizada, estatus, modalidad, fecha_realizacion,
            hora_conclusion, nombre_recibe, soporte_entrega, anio_archivado, folio_anual
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        if ($stmt_insert->execute()) {
            $conn->commit();
            // Redirigir al histórico con el año elegido
            header('Location: historico_servicios.php?year=' . $anio_archivado . '&mensaje=' . urlencode('Servicio agregado al histórico correctamente (folio #' . $folio_anual . ')'));
            exit;
        } else {
            $conn->rollback();
            error_log('Error al guardar en histórico: ' . $conn->error);
...
<head>
    ...
    <title>Agregar al Histórico</title>
...
<body>
    <div class="container">
        <div class="header-form">
            <h1>📜 Agregar al Histórico</h1>
            <a href="gestion_servicio.php" class="btn-volver">⬅ Volver</a>
        </div>
        ...
        <form method="POST" action="">
            ...
            <div class="seccion-form">
                ...
                <div class="form-grid">
                    ...
                    <!-- 👇 CAMPO EXTRA PARA ELEGIR EL AÑO DEL HISTÓRICO 👇 -->
                    <div class="campo">
                        <label for="anio_archivado">📅 Año a Archivar (Histórico) <span class="obligatorio">*</span></label>
                        <select id="anio_archivado" name="anio_archivado" required>
            ...
            <div class="seccion-form">
                ...
                <!-- PANEL AMARILLO PARA REPARACIÓN TEMPORAL -->
            ...
            <div class="acciones-form">
                ...
                <button type="submit" class="btn btn-guardar">📜 Agregar al Histórico</button>
            </div>
        </form>

    </div>

    <!-- SCRIPT PARA DESPLEGAR EL PANEL AMARILLO (Igual que en nuevo_servicio.php) -->
    <script>
