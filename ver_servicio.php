...
if ($id == 0) {
    header('Location: gestion_servicio.php?error=ID no válido');
    exit;
}
...
if ($result->num_rows == 0) {
    header('Location: gestion_servicio.php?error=Servicio no encontrado');
    exit;
}
...
<head>
    ...
    <title>Detalle del Servicio - Folio <?php echo $folio_mostrar; ?></title>
    ...
    <style>
        ...
        .header-detalle { display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 15px; border-bottom: 3px solid #2e7d32; padding-bottom: 15px; margin-bottom: 25px; }
        .header-detalle h1 { color: #2e7d32; font-size: 24px; }
        .header-detalle .folio-badge { background: #2e7d32; color: #fff; padding: 8px 25px; border-radius: 20px; font-weight: 700; font-size: 18px; }
        ...
        .btn-imprimir { background: #17a2b8; color: #fff; padding: 10px 25px; border-radius: 8px; text-decoration: none; font-weight: 600; transition: all 0.3s; border: none; cursor: pointer; }
        .btn-imprimir:hover { background: #138496; transform: translateY(-2px); }
        .seccion-detalle { background: #f8f9fa; border-radius: 10px; padding: 20px; margin-bottom: 20px; border-left: 4px solid #2e7d32; }
        .seccion-detalle h3 { color: #2e7d32; font-size: 16px; margin-bottom: 15px; border-bottom: 1px solid #e6eaef; padding-bottom: 10px; }
...
<body>
    <div class="container">
        <div class="header-detalle">
            <div><h1>📋 Detalle del Servicio</h1></div>
        ...
        <div class="acciones-botones no-print">
            <a href="editar_servicio.php?id=<?php echo $servicio['id']; ?>" class="btn btn-editar" style="background:#ffc107; color:#1e2a3a; padding:10px 25px; border-radius:8px; text-decoration:none; font-weight:600;">✏️ Editar</a>
            <button onclick="window.print()" class="btn btn-imprimir" style="background:#17a2b8; color:#fff; padding:10px 25px; border-radius:8px; border:none; font-weight:600; cursor:pointer;">🖨️ Imprimir</button>
            <a href="gestion_servicio.php" class="btn btn-volver" style="background:#6c757d; color:#fff; padding:10px 25px; border-radius:8px; text-decoration:none; font-weight:600;">⬅ Volver al Listado</a>
        </div>

        <div class="footer-detalle">
            <p>Reporte generado el <?php echo date('d/m/Y H:i'); ?> - Sistema de Gestión de Servicios</p>
